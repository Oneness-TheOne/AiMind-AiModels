...
def get_tree_interpretation_prompt(analysis_result):
    ...
    ## 출력 형식:
    다음 JSON 형식으로 응답해주세요 (대시보드 탭 구조와 1:1로 매핑되도록 작성):
    {{
        "전체_요약": "한 문단으로 전체 그림의 핵심 소견을 요약",
        "형식_및_시각_분석": {{
            "위치_및_크기": "위치·크기를 수치 없이 '작게/보통/크게', '중앙/왼쪽/오른쪽' 등으로만 서술한 형식적 해석",
            "필압_및_형태": "선의 강도, 필압, 형태적 특징(곡선/각진 형태 등)에 대한 해석"
        }},
        "발달_단계_보정": {{
            "정상_발달_특징": "{age}세 발달 단계에서 '정상 범위'로 볼 수 있는 특징을 요약",
            "주의_관찰_특징": "연령 대비 추가 관찰이 필요하거나 경계선에 있는 발달적 특징을 요약"
        }},
        "심리_임상_소견": {{
            "정서_위기_지표": "우울·불안 등 정서 위기와 관련된 지표에 대한 임상적 소견을 요약",
            "성격_표준화_점수": "성격 5요인 등 성격 특성에 대한 표준화된 해석을 요약",
            "핵심_상징_해석": "수관·기둥·가지·뿌리 등 핵심 상징이 보여주는 자아상·대인관계·환경 인식에 대한 해석"
        }},
        "종합_제언": {{
            "임상_종합_소견": "전반적인 임상 종합 소견을 한 단락으로 기술 (따옴표 포함 인용문 형태여도 좋음)",
            "부모_강점_가이드": "부모/양육자를 위한 강점 활용 및 양육 가이드를 1~3가지 핵심 메시지로 요약",
            "전문가_분석_코멘트": "전문가 코멘트 형식으로 추가적인 해석·주의점·관찰 권고를 제시"
        }}
    }}
...
def get_person_interpretation_prompt(analysis_result, person_type="남자사람"):
    ...
    {{
        ...
        "구성_분석": {{
            "위치_및_크기": "사람의 위치와 크기를 수치 없이 '작게/보통/크게', '중앙/왼쪽/오른쪽' 등으로만 서술한 해석",
...
def get_house_interpretation_prompt(analysis_result):
    ...
    {{
        "전체_요약": "한 문단으로 전체 그림의 특징과 주요 해석",
        "구성_분석": {{
            "위치_및_크기": "집의 위치와 크기를 수치 없이 '작게/보통/크게', '중앙/왼쪽/오른쪽' 등으로만 서술한 해석",
